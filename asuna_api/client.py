...
class Client:
    ...
    async def mc_user(self, username=None):
        ...
        if isinstance(response, dict):
            api_response = await self._http_client.get(self.mchistory_uuid(response.get("id")))

            for json in api_response:
                if "changedToAt" in json.keys():
                    json["timeChangedAt"] = datetime.datetime.utcfromtimestamp(int(json["changedToAt"]) / 1000)

                    json["changedToAt"] = datetime.datetime.utcfromtimestamp(int(json["changedToAt"]) / 1000)

                if not "changedToAt" in json.keys():
                    json["changedToAt"] = "Original Name"

            mc_data = {"username": response["name"], "uuid": response["id"], "name_history": api_response}
    ...
    def mchistory_username(self, username):
        url = URL.build(scheme="https", host="api.mojang.com/users/profiles/minecraft", path=f"/{username.lstrip('/')}")
        return str(url)
